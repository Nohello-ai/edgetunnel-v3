...
`)}`),g}async function Se(e,t,r,o,i,s,n,a=null,l={},f=!1,c=null){let p=l.反代IP||"",g=l.代理类型!==void 0?l.代理类型:null,h=l.代理全局!==void 0?l.代理全局:!1,w=l.代理参数||{},u=l.反代兜底!==void 0?l.反代兜底:!0,d=0;O(`[TCP转发] 目标: ${e}:${t} | 反代IP: ${p} | 反代兜底: ${u?"是":"否"} | 反代类型: ${g||"proxyip"} | 全局: ${h?"是":"否"}`);let b=1e3,$=!1,S=Fe(a),m=f&&(l.木马反代地址||null),v=m?l.木马反代地址:null,x=m?vn(c,r):null;async function y(A,L=b){let U;try{await Promise.race([A.opened,new Promise((R,I)=>{U=setTimeout(()=>I(new Error("连接超时")),L)})])}finally{clearTimeout(U)}}async function C(A,L){let U=S({hostname:A,port:L});try{return await y(U),U}catch(R){try{U?.close?.()}catch{}throw R}}async function k(A,L){if(q(L)<=0)return;let U=A.writable.getWriter();try{await U.write(M(L))}finally{try{U.releaseLock()}catch{}}}async function E(A){if(A.length===1){let R=A[0];return{socket:await C(R.hostname,R.port),candidate:R}}let L=A.map(R=>C(R.hostname,R.port).then(I=>({socket:I,candidate:R}))),U=null;try{return U=await Promise.any(L),U}finally{if(U)for(let R of L)R.then(({socket:I})=>{if(I!==U.socket)try{I?.close?.()}catch{}}).catch(()=>{})}}async function T(A,L){if(!At()||It(A))return null;O(`[TCP直连] 预加载竞速拨号开启，开始并发查询 ${A} 的 A/AAAA 记录`);let[U,R]=await Promise.all([Te(A,"A"),Te(A,"AAAA")]),I=[...new Set(U.flatMap(j=>{let W=j.data;return j.type===1&&typeof W=="string"&&et(W)?[W]:[]}))],N=[...new Set(R.flatMap(j=>{let W=j.data;return j.type===28&&typeof W=="string"&&It(W)?[W]:[]}))],F=Math.max(1,Je()|0),H=I.length>=F?I.slice(0,F):I.concat(N.slice(0,F-I.length)),X=I.length>0?H.length>I.length?"A+AAAA":"A":"AAAA";if(H.length===0)return O(`[TCP直连] ${A} 的 A/AAAA 未获得可用解析结果，预加载竞速不可用，回退到原始 hostname 直连。`),null;let te=H;return O(`[TCP直连] ${A} A记录:${I.length} AAAA记录:${N.length}，使用${X}记录，竞速拨号 ${te.length}/${F}: ${te.join(", ")}`),te.map((j,W)=>({hostname:j,port:L,attempt:W,resolvedFrom:A}))}async function P(A,L,U=null,R=!1){let I=R?await T(A,L):null,N=I||Array.from({length:Je()},(H,X)=>({hostname:A,port:L,attempt:X}));O(I?`[TCP直连] 并发尝试 ${N.length} 路: ${N.map(H=>`${H.hostname}:${H.port}`).join(", ")}`:`[TCP直连] 并发尝试 ${N.length} 路: ${A}:${L}`);let F=null;try{let H=await E(N);if(F=H.socket,I){let X=H.candidate;O(`[TCP直连] 预加载竞速结果: ${X.hostname}:${X.port} 胜出，源域名: ${X.resolvedFrom||A}`)}return await k(F,U),F}catch(H){try{F?.close?.()}catch{}throw I&&O(`[TCP直连] 预加载竞速失败: ${H.message||H}`),H}}async function _(A,L,U=null,R=null,I=!0){if(R&&R.length>0){let N=Math.max(1,Math.floor(Number(Tt())||1));for(let F=0;F<R.length;F+=N){let H=[];for(let j=0;j<N&&F+j<R.length;j++){let W=(d+F+j)%R.length,[re,Z]=R[W];H.push({hostname:re,port:Z,index:W})}let X=null,te=null;try{O(`[反代连接] 并发尝试 ${H.length} 路: ${H.map(W=>`${W.hostname}:${W.port}`).join(", ")}`);let j=await E(H);return X=j.socket,te=j.candidate,await k(X,U),O(`[反代连接] 成功连接到: ${te.hostname}:${te.port} (索引: ${te.index})`),d=te.index,X}catch(j){try{X?.close?.()}catch{}O(`[反代连接] 本批连接失败: ${j.message||j}`)}}}if(I)return P(A,L,U,!1);throw ee(o),new Error("[反代连接] 所有反代连接失败，且未启用反代兜底，连接终止。")}async function z(A=!0){if(s.connectingPromise){await s.connectingPromise;return}let L=!1,U=null;m?A&&!$&&q(c)>0?(U=c,L=q(r)>0):U=x:(L=A&&!$&&q(r)>0,U=L?r:null);let R=(async()=>{let I;if(m)O(`[木马反代] 代理到: ${e}:${t}`),I=await _r(U,S,v);else if(g==="socks5")O(`[SOCKS5代理] 代理到: ${e}:${t}`),I=await Ut(e,t,U,S,w);else if(g==="http")O(`[HTTP代理] 代理到: ${e}:${t}`),I=await dt(e,t,U,!1,S,w);else if(g==="https")O(`[HTTPS代理] 代理到: ${e}:${t}`),I=await dt(e,t,U,!0,S,w);else if(g==="turn"){if(O(`[TURN代理] 代理到: ${e}:${t}`),I=await Nt(w,e,t,S),q(U)>0){let N=I.writable.getWriter();try{await N.write(M(U))}finally{try{N.releaseLock()}catch{}}}}else{O(`[反代连接] 代理到: ${e}:${t}`);let N=await $n(p,e,n);I=await _(`${se[0]}.tp1.${se[2]}.xyz`,1,U,N,u)}L&&($=!0),s.socket=I,I.closed.catch(()=>{}).finally(()=>ee(o)),ft(I,o,i,null)})();s.connectingPromise=R;try{await R}finally{s.connectingPromise===R&&(s.connectingPromise=null)}}if(s.retryConnect=async()=>z(!$),g&&(h||je().some(A=>new RegExp(`^${A.replace(/\*/g,".*")}$`,"i").test(e)))){O("[TCP转发] 启用 SOCKS5/HTTP/HTTPS/TURN 全局代理");try{await z()}catch(A){throw O(`[TCP转发] SOCKS5/HTTP/HTTPS/TURN 代理连接失败: ${A.message}`),A}}else try{O(`[TCP转发] 尝试直连到: ${e}:${t}`);let A=await P(e,t,r,!0);s.socket=A,ft(A,o,i,async()=>{s.socket===A&&await z()})}catch(A){if(O(`[TCP转发] 直连 ${e}:${t} 失败: ${A.message}`),A instanceof Error&&A.name==="预加载解析为空")throw ee(o),A;await z()}}async function Bo(e){let t=new Uint8Array(2),r=0,o=null,i=0;for(;;){let{done:s,value:n}=await e.read();if(s||!n)throw new Error("DNS over TCP response is truncated");let a=M(n),l=0;if(r<2){let c=Math.min(2-r,a.byteLength);if(t.set(a.subarray(0,c),r),r+=c,l+=c,r<2)continue;let p=t[0]<<8|t[1];if(p===0)throw new Error("DNS over TCP response has empty payload");o=new Uint8Array(2+p),o.set(t,0),i=2}let f=Math.min(o.byteLength-i,a.byteLength-l);if(f>0&&(o.set(a.subarray(l,l+f),i),i+=f),i===o.byteLength)return o}}async function $e(e,t,r,o,i=null,s=null){let n=M(e),a=n.byteLength;O(`[UDP转发] 收到 DNS 请求: ${a}B -> 8.8.4.4:53`);let l=null,f=null,c=null;try{l=(s||Fe(o))({hostname:"8.8.4.4",port:53});let g=r;f=l.writable.getWriter(),await f.write(n),O(`[UDP转发] DNS 请求已写入上游: ${a}B`),f.releaseLock(),f=null,c=l.readable.getReader();let h=await Bo(c);O(`[UDP转发] 收到完整 DNS 响应: ${h.byteLength}B`);let w=i?await i(h):h,u=Array.isArray(w)?w:[w];if(!u.length||t.readyState!==WebSocket.OPEN)return;for(let d of u){let b=M(d);if(b.byteLength)if(g){let $=new Uint8Array(g.length+b.byteLength);$.set(g,0),$.set(b,g.length),await Ze(t,$.buffer),g=null}else await Ze(t,b)}}catch(p){O(`[UDP转发] DNS 转发失败: ${p?.message||p}`)}finally{try{f?.releaseLock()}catch{}try{c?.releaseLock()}catch{}try{await Promise.resolve(l?.close?.())}catch{}}}var Ho=new TextDecoder;function Sn(e){let t=String(e||"").trim();if(!t||t.includes("/")||t.includes("@")||t.includes("://"))throw new Error("木马反代仅支持 host:port");let r="",o="";if(t.startsWith("[")){let s=t.match(/^(\[[^\]]+\]):(\d+)$/);if(!s)throw new Error("无效的 IPv6 木马反代地址");r=s[1],o=s[2]}else{let s=t.split(":");if(s.length!==2)throw new Error("木马反代仅支持 host:port");r=s[0],o=s[1]}let i=Number(o);if(!r||!Number.isInteger(i)||i<1||i>65535)throw new Error("无效的木马反代端口");return{hostname:r,port:i}}async function _r(e,t,r){if(!r)throw new Error("trojan fallback is not configured");let o=t({hostname:qe(r.hostname),port:r.port}),i=null;try{return o.opened&&await o.opened,q(e)>0&&(i=o.writable.getWriter(),await i.write(M(e))),o}catch(s){try{o?.close?.()}catch{}throw s}finally{try{i?.releaseLock()}catch{}}}function vn(e,t){let r=M(e),o=M(t);if(!o.byteLength)return r;let i=r.byteLength-o.byteLength;if(i<=0)return r;for(let s=0;s<o.byteLength;s++)if(r[i+s]!==o[s])return r;return r.subarray(0,i)}async function Ko(e,t,r,o){let i=M(e);if(!r.反代Socket){let n=Fe(o),a=await _r(i,n,r.反代地址);r.反代Socket=a,a.closed.catch(()=>{}).finally(()=>ee(t)),ft(a,t,null,null);return}if(!i.byteLength)return;let s=r.反代Socket.writable.getWriter();try{await s.write(i)}finally{try{s.releaseLock()}catch{}}}async function he(e,t,r,o){let i=M(e);if(r?.反代地址)return Ko(i,t,r,o);let s=r?.缓存 instanceof Uint8Array?r.缓存:new Uint8Array(0),n=s.byteLength?ie(s,i):i,a=0;for(;a<n.byteLength;){let l=a,f=n[a],c=a+1,p=0;if(f===1)p=4;else if(f===4)p=16;else if(f===3){if(n.byteLength<c+1)break;p=1+n[c]}else throw new Error(`invalid trojan udp addressType: ${f}`);let g=c+p;if(n.byteLength<g+6)break;let h=n[g]<<8|n[g+1],w=n[g+2]<<8|n[g+3];if(n[g+4]!==13||n[g+5]!==10)throw new Error("invalid trojan udp delimiter");let u=g+6,d=u+w;if(n.byteLength<d)break;let b=n.slice(l,g+2),$=n.slice(u,d);if(a=d,h!==53)throw new Error("UDP is not supported");if(!$.byteLength)continue;let S=$;($.byteLength<2||($[0]<<8|$[1])!==$.byteLength-2)&&(S=new Uint8Array($.byteLength+2),S[0]=$.byteLength>>>8&255,S[1]=$.byteLength&255,S.set($,2));let m={缓存:new Uint8Array(0)};await $e(S,t,null,o,v=>{let x=M(v),y=m.缓存.byteLength?ie(m.缓存,x):x,C=[],k=0;for(;k+2<=y.byteLength;){let E=y[k]<<8|y[k+1],T=k+2,P=T+E;if(P>y.byteLength)break;let _=y.slice(T,P),z=new Uint8Array(b.byteLength+4+_.byteLength);z.set(b,0),z[b.byteLength]=_.byteLength>>>8&255,z[b.byteLength+1]=_.byteLength&255,z[b.byteLength+2]=13,z[b.byteLength+3]=10,z.set(_,b.byteLength+4),C.push(z),k=P}return m.缓存=y.slice(k),C.length?C:new Uint8Array(0)})}r&&(r.缓存=n.slice(a))}function Dt(e,t){let r=M(e),o=Ye(t);if(r.byteLength<58)return{hasError:!0,message:"invalid data"};let i=56;if(r[i]!==13||r[i+1]!==10)return{hasError:!0,message:"invalid header format"};for(let w=0;w<i;w++)if(r[w]!==o.charCodeAt(w))return{hasError:!0,message:"invalid password"};let s=i+2;if(r.byteLength<s+6)return{hasError:!0,message:"invalid S5 request data"};let n=r[s];if(n!==1&&n!==3)return{hasError:!0,message:"unsupported command, only TCP/UDP is allowed"};let a=n===3,l=r[s+1],f=0,c=s+2,p="";switch(l){case 1:if(f=4,r.byteLength<c+f+4)return{hasError:!0,message:"invalid S5 request data"};p=`${r[c]}.${r[c+1]}.${r[c+2]}.${r[c+3]}`;break;case 3:if(r.byteLength<c+1)return{hasError:!0,message:"invalid S5 request data"};if(f=r[c],c+=1,r.byteLength<c+f+4)return{hasError:!0,message:"invalid S5 request data"};p=Ho.decode(r.subarray(c,c+f));break;case 4:if(f=16,r.byteLength<c+f+4)return{hasError:!0,message:"invalid S5 request data"};let w=[];for(let u=0;u<8;u++){let d=c+u*2;w.push((r[d]<<8|r[d+1]).toString(16))}p=w.join(":");break;default:return{hasError:!0,message:`invalid addressType is ${l}`}}if(!p)return{hasError:!0,message:`address is empty, addressType is ${l}`};let g=c+f;if(r.byteLength<g+4)return{hasError:!0,message:"invalid S5 request data"};let h=r[g]<<8|r[g+1];return{hasError:!1,addressType:l,port:h,hostname:p,isUDP:a,rawClientData:r.subarray(g+4)}}function Mt(e={}){let t=e.传输协议==="grpc";return{type:t?e.gRPC模式==="multi"?"grpc&mode=multi":"grpc&mode=gun":e.传输协议==="xhttp"?"xhttp&mode=stream-one":"ws",路径字段名:t?"serviceName":"path",域名字段名:t?"authority":"host"}}function jt(e={},t="/",r=!1){let o=r?"/":e.随机路径?Qe(t):t;return e.传输协议!=="grpc"?o:o.split("?")[0]||"/"}async function $r(e,t,r="",o=!0){let{searchParams:i}=e,s=ve(e.pathname),n=s.toLowerCase(),a=r,l=null,f=!1,c="",p={},g=o,h={木马反代地址:null,反代IP:a,代理类型:null,代理账号:"",代理全局:!1,代理参数:{},反代兜底:g},w=()=>{h.反代IP=a,h.代理类型=l,h.代理账号=c,h.代理全局=f,h.代理参数={...p},h.反代兜底=g},u=s.match(/\/video\/(.+)$/i);if(u)try{let v=fn(u[1],t),{type:x,...y}=JSON.parse(v);if(!x||!kn[String(x).toLowerCase()])throw new Error("链式代理类型无效");if(!y.hostname||!y.port)throw new Error("链式代理地址缺少 hostname 或 port");let C=Re(y.port,"链式代理端口"),{hostname:k}=lt(y.hostname,C);return c="",a="链式代理",g=!1,f=!0,l=String(x).toLowerCase(),p={username:y.username,password:y.password,hostname:k,port:C},w(),h}catch(v){console.error("解析链式代理参数失败:",v.message)}c=i.get("socks5")||i.get("http")||i.get("https")||i.get("turn")||null,f=i.has("globalproxy"),i.get("socks5")?l="socks5":i.get("http")?l="http":i.get("https")?l="https":i.get("turn")&&(l="turn");let d=(v,x=!0)=>{let y=/^(socks5|http|https|turn):\/\/(.+)$/i.exec(v||"");return y?(l=y[1].toLowerCase(),c=y[2].split("/")[0],x&&(f=!0),!0):!1},b=v=>{a=v,l=null,g=!1},$=v=>{if(!v.includes("://")){let C=v.indexOf("/");return C>0?v.slice(0,C):v}let x=v.split("://");if(x.length!==2)return v;let y=x[1].indexOf("/");return y>0?`${x[0]}://${x[1].slice(0,y)}`:v},S=/\/trojan=([^?#\s]+)/i.exec(s);if(S)try{h.木马反代地址=Sn(S[1])}catch(v){console.error("解析木马反代地址失败:",v.message),h.木马反代地址=null}let m=i.get("proxyip");if(m!==null){if(!d(m))return b(m),w(),h}else{let v=/\/(socks5?|http|https|turn):\/?\/?([^/?#\s]+)/i.exec(s);if(v){let x=v[1].toLowerCase();l=x==="sock"||x==="socks"?"socks5":x,c=v[2].split("/")[0],f=!0}else if(v=/\/(g?s5|socks5|g?http|g?https|g?turn)=([^/?#\s]+)/i.exec(s)){let x=v[1].toLowerCase();c=v[2].split("/")[0],l=x.includes("turn")?"turn":x.includes("https")?"https":x.includes("http")?"http":"socks5",x.startsWith("g")&&(f=!0)}else if(v=/\/(proxyip[.=]|pyip=|ip=)([^?#\s]+)/.exec(n)){let x=$(v[2]);if(!d(x))return b(x),w(),h}}if(!c)return l=null,w(),h;try{p=await gt(c,mt(l)),i.get("socks5")?l="socks5":i.get("http")?l="http":i.get("https")?l="https":i.get("turn")?l="turn":l=l||"socks5"}catch(v){console.error("解析SOCKS5地址失败:",v.message),l=null}return w(),h}var kn={socks5:1080,http:80,https:443,turn:3478};function mt(e){return kn[String(e||"").toLowerCase()]||80}var Wo=/^(?:[A-Z0-9+/]{4})*(?:[A-Z0-9+/]{2}==|[A-Z0-9+/]{3}=)?$/i;function gt(e,t=80){e=String(e||"").trim().replace(/^(socks5|http|https|turn):\/\//i,"").split("#")[0].trim();let r=e.lastIndexOf("@");if(r!==-1){let p=e.slice(0,r).replace(/%3D/ig,"=");!p.includes(":")&&Wo.test(p)&&(p=atob(p)),e=`${p}@${e.slice(r+1)}`}let o=e.lastIndexOf("@"),i=(o===-1?e:e.slice(o+1)).split("/")[0],s=o===-1?"":e.slice(0,o),n=s.indexOf(":"),a=n===-1?void 0:ve(s.slice(0,n)),l=n===-1?void 0:ve(s.slice(n+1));if(s&&(n<1||!l))throw new Error('无效的 SOCKS 地址格式：认证部分必须是 "username:password" 的形式');let f,c;try{({hostname:f,port:c}=lt(i,t))}catch(p){throw new Error(`无效的 SOCKS 地址格式：${p.message}`)}return{username:a,password:l,hostname:f,port:c}}async function Ft(e,t,r,o){let i="https://api.cloudflare.com/client/v4",s=a=>a?.reduce((l,f)=>l+(f?.sum?.requests||0),0)||0,n={"Content-Type":"application/json"};try{if(!r&&(!e||!t))return{success:!1,pages:0,workers:0,total:0,max:1e5};if(!r){let d=await fetch(`${i}/accounts`,{method:"GET",headers:{...n,"X-AUTH-EMAIL":e,"X-AUTH-KEY":t}});if(!d.ok)throw new Error(`账户获取失败: ${d.status}`);let b=await d.json();if(!b?.result?.length)throw new Error("未找到账户");let $=b.result.findIndex(S=>S.name?.toLowerCase().startsWith(e.toLowerCase()));r=b.result[$>=0?$:0]?.id}let a=new Date;a.setUTCHours(0,0,0,0);let l=o?{...n,Authorization:`Bearer ${o}`}:{...n,"X-AUTH-EMAIL":e,"X-AUTH-KEY":t},f=await fetch(`${i}/graphql`,{method:"POST",headers:l,body:JSON.stringify({query:`query getBillingMetrics($AccountID: String!, $filter: AccountWorkersInvocationsAdaptiveFilter_InputObject) {
					viewer { accounts(filter: {accountTag: $AccountID}) {
						pagesFunctionsInvocationsAdaptiveGroups(limit: 1000, filter: $filter) { sum { requests } }
						workersInvocationsAdaptive(limit: 10000, filter: $filter) { sum { requests } }
					} }
				}`,variables:{AccountID:r,filter:{datetime_geq:a.toISOString(),datetime_leq:new Date().toISOString()}}})});if(!f.ok)throw new Error(`查询失败: ${f.status}`);let c=await f.json();if(c.errors?.length)throw new Error(c.errors[0].message);let p=c?.data?.viewer?.accounts?.[0];if(!p)throw new Error("未找到账户数据");let g=s(p.pagesFunctionsInvocationsAdaptiveGroups),h=s(p.workersInvocationsAdaptive),w=g+h,u=1e5;return O(`统计结果 - Pages: ${g}, Workers: ${h}, 总计: ${w}, 上限: 100000`),{success:!0,pages:g,workers:h,total:w,max:u}}catch(a){return console.error("获取使用量错误:",a.message),{success:!1,pages:0,workers:0,total:0,max:1e5}}}var Cn;function K(e,t,r){function o(a,l){if(a._zod||Object.defineProperty(a,"_zod",{value:{def:l,constr:n,traits:new Set},enumerable:!1}),a._zod.traits.has(e))return;a._zod.traits.add(e),t(a,l);let f=n.prototype,c=Object.keys(f);for(let p=0;p<c.length;p++){let g=c[p];g in a||(a[g]=f[g].bind(a))}}let i=r?.Parent??Object;class s extends i{}Object.defineProperty(s,"name",{value:e});function n(a){var l;let f=r?.Parent?new s:this;o(f,a),(l=f._zod).deferred??(l.deferred=[]);for(let c of f._zod.deferred)c();return f}return Object.defineProperty(n,"init",{value:o}),Object.defineProperty(n,Symbol.hasInstance,{value:a=>r?.Parent&&a instanceof r.Parent?!0:a?._zod?.traits?.has(e)}),Object.defineProperty(n,"name",{value:e}),n}var ea=Symbol("zod_brand"),Ae=class extends Error{constructor(){super("Encountered Promise during synchronous parse. Use .parseAsync() instead.")}};(Cn=globalThis).__zod_globalConfig??(Cn.__zod_globalConfig={});var vr=globalThis.__zod_globalConfig;function rt(e){return e&&Object.assign(vr,e),vr}function zn(e){let t=Object.values(e).filter(o=>typeof o=="number");return Object.entries(e).filter(([o,i])=>t.indexOf(+o)===-1).map(([o,i])=>i)}function Tn(e,t){return typeof t=="bigint"?t.toString():t}function An(e){return{get value(){{let r=e();return Object.defineProperty(this,"value",{value:r}),r}throw new Error("cached value already set")}}}function En(e){return e==null}function In(e){let t=e.startsWith("^")?1:0,r=e.endsWith("$")?e.length-1:e.length;return e.slice(t,r)}var Pn=Symbol("evaluating");function Ee(e,t,r){let o;Object.defineProperty(e,t,{get(){if(o!==Pn)return o===void 0&&(o=Pn,o=r()),o},set(i){Object.defineProperty(e,t,{value:i})},configurable:!0})}var kr="captureStackTrace"in Error?Error.captureStackTrace:(...e)=>{};function Ln(e){return typeof e=="object"&&e!==null&&!Array.isArray(e)}var Un=new Set(["string","number","symbol"]);function Cr(e){return e.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")}function Pr(e,t,r){let o=new e._zod.constr(t??e._zod.def);return(!t||r?.parent)&&(o._zod.parent=e),o}function me(e){let t=e;if(!t)return{};if(typeof t=="string")return{error:()=>t};if(t?.message!==void 0){if(t?.error!==void 0)throw new Error("Cannot specify both `message` and `error` params");t.error=t.message}return delete t.message,typeof t.error=="string"?{...t,error:()=>t.error}:t}function Rn(e){return Object.keys(e).filter(t=>e[t]._zod.optin==="optional"&&e[t]._zod.optout==="optional")}var Zn={safeint:[Number.MIN_SAFE_INTEGER,Number.MAX_SAFE_INTEGER],int32:[-2147483648,2147483647],uint32:[0,4294967295],float32:[-34028234663852886e22,34028234663852886e22],float64:[-Number.MAX_VALUE,Number.MAX_VALUE]};function yt(e,t=0){if(e.aborted===!0)return!0;for(let r=t;r<e.issues.length;r++)if(e.issues[r]?.continue!==!0)return!0;return!1}function On(e,t=0){if(e.aborted===!0)return!0;for(let r=t;r<e.issues.length;r++)if(e.issues[r]?.continue===!1)return!0;return!1}function Ht(e,t){return t.map(r=>{var o;return(o=r).path??(o.path=[]),r.path.unshift(e),r})}function Bt(e){return typeof e=="string"?e:e?.message}function nt(e,t,r){let o=e.message?e.message:Bt(e.inst?._zod.def?.error?.(e))??Bt(t?.error?.(e))??Bt(r.customError?.(e))??Bt(r.localeError?.(e))??"Invalid input",{inst:i,continue:s,input:n,...a}=e;return a.path??(a.path=[]),a.message=o,t?.reportInput&&(a.input=n),a}function Nn(e){return Array.isArray(e)?"array":typeof e=="string"?"string":"unknown"}var Dn=(e,t)=>{e.name="$ZodError",Object.defineProperty(e,"_zod",{value:e._zod,enumerable:!1}),Object.defineProperty(e,"issues",{value:t,enumerable:!1}),e.message=JSON.stringify(t,Tn,2),Object.defineProperty(e,"toString",{value:()=>e.message,enumerable:!1})},Mn=K("$ZodError",Dn),wt=K("$ZodError",Dn,{Parent:Error});var Vo=e=>(t,r,o,i)=>{let s=o?{...o,async:!1}:{async:!1},n=t._zod.run({value:r,issues:[]},s);if(n instanceof Promise)throw new Ae;if(n.issues.length){let a=new(i?.Err??e)(n.issues.map(l=>nt(l,s,rt())));throw kr(a,i?.callee),a}return n.value},Kt=Vo(wt),Jo=e=>async(t,r,o,i)=>{let s=o?{...o,async:!0}:{async:!0},n=t._zod.run({value:r,issues:[]},s);if(n instanceof Promise&&(n=await n),n.issues.length){let a=new(i?.Err??e)(n.issues.map(l=>nt(l,s,rt())));throw kr(a,i?.callee),a}return n.value},Wt=Jo(wt),Xo=e=>(t,r,o)=>{let i=o?{...o,async:!1}:{async:!1},s=t._zod.run({value:r,issues:[]},i);if(s instanceof Promise)throw new Ae;return s.issues.length?{success:!1,error:new(e??Mn)(s.issues.map(n=>nt(n,i,rt())))}:{success:!0,data:s.value}},xt=Xo(wt),Yo=e=>async(t,r,o)=>{let i=o?{...o,async:!0}:{async:!0},s=t._zod.run({value:r,issues:[]},i);return s instanceof Promise&&(s=await s),s.issues.length?{success:!1,error:new e(s.issues.map(n=>nt(n,i,rt())))}:{success:!0,data:s.value}},bt=Yo(wt);var Qo="(?:(?:\\d\\d[2468][048]|\\d\\d[13579][26]|\\d\\d0[48]|[02468][048]00|[13579][26]00)-02-29|\\d{4}-(?:(?:0[13578]|1[02])-(?:0[1-9]|[12]\\d|3[01])|(?:0[469]|11)-(?:0[1-9]|[12]\\d|30)|(?:02)-(?:0[1-9]|1\\d|2[0-8])))",qo=new RegExp(`^${Qo}$`);var jn=e=>{let t=e?`[\\s\\S]{${e?.minimum??0},${e?.maximum??""}}`:"[\\s\\S]*";return new RegExp(`^${t}$`)};var Fn=/^-?\d+$/,Bn=/^-?\d+(?:\.\d+)?$/,Hn=/^(?:true|false)$/i;var Ke=K("$ZodCheck",(e,t)=>{var r;e._zod??(e._zod={}),e._zod.def=t,(r=e._zod).onattach??(r.onattach=[])}),Kn={number:"number",bigint:"bigint",object:"date"},Wn=K("$ZodCheckLessThan",(e,t)=>{Ke.init(e,t);let r=Kn[typeof t.value];e._zod.onattach.push(o=>{let i=o._zod.bag,s=(t.inclusive?i.maximum:i.exclusiveMaximum)??Number.POSITIVE_INFINITY;t.value<s&&(t.inclusive?i.maximum=t.value:i.exclusiveMaximum=t.value)}),e._zod.check=o=>{(t.inclusive?o.value<=t.value:o.value<t.value)||o.issues.push({origin:r,code:"too_big",maximum:typeof t.value=="object"?t.value.getTime():t.value,input:o.value,inclusive:t.inclusive,inst:e,continue:!t.abort})}}),Tr=K("$ZodCheckGreaterThan",(e,t)=>{Ke.init(e,t);let r=Kn[typeof t.value];e._zod.onattach.push(o=>{let i=o._zod.bag,s=(t.inclusive?i.minimum:i.exclusiveMinimum)??Number.NEGATIVE_INFINITY;t.value>s&&(t.inclusive?i.minimum=t.value:i.exclusiveMinimum=t.value)}),e._zod.check=o=>{(t.inclusive?o.value>=t.value:o.value>t.value)||o.issues.push({origin:r,code:"too_small",minimum:typeof t.value=="object"?t.value.getTime():t.value,input:o.value,inclusive:t.inclusive,inst:e,continue:!t.abort})}});var Gn=K("$ZodCheckNumberFormat",(e,t)=>{Ke.init(e,t),t.format=t.format||"float64";let r=t.format?.includes("int"),o=r?"int":"number",[i,s]=Zn[t.format];e._zod.onattach.push(n=>{let a=n._zod.bag;a.format=t.format,a.minimum=i,a.maximum=s,r&&(a.pattern=Fn)}),e._zod.check=n=>{let a=n.value;if(r){if(!Number.isInteger(a)){n.issues.push({expected:o,format:t.format,code:"invalid_type",continue:!1,input:a,inst:e});return}if(!Number.isSafeInteger(a)){a>0?n.issues.push({input:a,code:"too_big",maximum:Number.MAX_SAFE_INTEGER,note:"Integers must be within the safe integer range.",inst:e,origin:o,inclusive:!0,continue:!t.abort}):n.issues.push({input:a,code:"too_small",minimum:Number.MIN_SAFE_INTEGER,note:"Integers must be within the safe integer range.",inst:e,origin:o,inclusive:!0,continue:!t.abort});return}}a<i&&n.issues.push({origin:"number",input:a,code:"too_small",minimum:i,inclusive:!0,inst:e,continue:!t.abort}),a>s&&n.issues.push({origin:"number",input:a,code:"too_big",maximum:s,inclusive:!0,inst:e,continue:!t.abort})}});var Vn=K("$ZodCheckMinLength",(e,t)=>{var r;Ke.init(e,t),(r=e._zod.def).when??(r.when=o=>{let i=o.value;return!En(i)&&i.length!==void 0}),e._zod.onattach.push(o=>{let i=o._zod.bag.minimum??Number.NEGATIVE_INFINITY;t.minimum>i&&(o._zod.bag.minimum=t.minimum)}),e._zod.check=o=>{let i=o.value;if(i.length>=t.minimum)return;let n=Nn(i);o.issues.push({origin:n,code:"too_small",minimum:t.minimum,inclusive:!0,input:i,inst:e,continue:!t.abort})}});var Xn={major:4,minor:4,patch:3};var ke=K("$ZodType",(e,t)=>{var r;e??(e={}),e._zod.def=t,e._zod.bag=e._zod.bag||{},e._zod.version=Xn;let o=[...e._zod.def.checks??[]];e._zod.traits.has("$ZodCheck")&&o.unshift(e);for(let i of o)for(let s of i._zod.onattach)s(e);if(o.length===0)(r=e._zod).deferred??(r.deferred=[]),e._zod.deferred?.push(()=>{e._zod.run=e._zod.parse});else{let i=(n,a,l)=>{let f=yt(n),c;for(let p of a){if(p._zod.def.when){if(On(n)||!p._zod.def.when(n))continue}else if(f)continue;let g=n.issues.length,h=p._zod.check(n);if(h instanceof Promise&&l?.async===!1)throw new Ae;if(c||h instanceof Promise)c=(c??Promise.resolve()).then(async()=>{await h,n.issues.length!==g&&(f||(f=yt(n,g)))});else{if(n.issues.length===g)continue;f||(f=yt(n,g))}}return c?c.then(()=>n):n},s=(n,a,l)=>{if(yt(n))return n.aborted=!0,n;let f=i(a,o,l);if(f instanceof Promise){if(l.async===!1)throw new Ae;return f.then(c=>e._zod.parse(c,l))}return e._zod.parse(f,l)};e._zod.run=(n,a)=>{if(a.skipChecks)return e._zod.parse(n,a);if(a.direction==="backward"){let f=e._zod.parse({value:n.value,issues:[]},{...a,skipChecks:!0});return f instanceof Promise?f.then(c=>s(c,n,a)):s(f,n,a)}let l=e._zod.parse(n,a);if(l instanceof Promise){if(a.async===!1)throw new Ae;return l.then(f=>i(f,o,a))}return i(l,o,a)}}Ee(e,"~standard",()=>({validate:i=>{try{let s=xt(e,i);return s.success?{value:s.data}:{issues:s.error?.issues}}catch{return bt(e,i).then(n=>n.success?{value:n.data}:{issues:n.error?.issues})}},vendor:"zod",version:1}))}),Qn=K("$ZodString",(e,t)=>{ke.init(e,t),e._zod.pattern=[...e?._zod.bag?.patterns??[]].pop()??jn(e._zod.bag),e._zod.parse=(r,o)=>{if(t.coerce)try{r.value=String(r.value)}catch{}return typeof r.value=="string"||r.issues.push({expected:"string",code:"invalid_type",input:r.value,inst:e}),r}});var Ar=K("$ZodNumber",(e,t)=>{ke.init(e,t),e._zod.pattern=e._zod.bag.pattern??Bn,e._zod.parse=(r,o)=>{if(t.coerce)try{r.value=Number(r.value)}catch{}let i=r.value;if(typeof i=="number"&&!Number.isNaN(i)&&Number.isFinite(i))return r;let s=typeof i=="number"?Number.isNaN(i)?"NaN":Number.isFinite(i)?void 0:"Infinity":void 0;return r.issues.push({expected:"number",code:"invalid_type",input:i,inst:e,...s?{received:s}:{}}),r}}),qn=K("$ZodNumberFormat",(e,t)=>{Gn.init(e,t),Ar.init(e,t)}),eo=K("$ZodBoolean",(e,t)=>{ke.init(e,t),e._zod.pattern=Hn,e._zod.parse=(r,o)=>{if(t.coerce)try{r.value=!!r.value}catch{}let i=r.value;return typeof i=="boolean"||r.issues.push({expected:"boolean",code:"invalid_type",input:i,inst:e}),r}});var to=K("$ZodUnknown",(e,t)=>{ke.init(e,t),e._zod.parse=r=>r});function Yn(e,t,r){e.issues.length&&t.issues.push(...Ht(r,e.issues)),t.value[r]=e.value}var ro=K("$ZodArray",(e,t)=>{ke.init(e,t),e._zod.parse=(r,o)=>{let i=r.value;if(!Array.isArray(i))return r.issues.push({expected:"array",code:"invalid_type",input:i,inst:e}),r;r.value=Array(i.length);let s=[];for(let n=0;n<i.length;n++){let a=i[n],l=t.element._zod.run({value:a,issues:[]},o);l instanceof Promise?s.push(l.then(f=>Yn(f,r,n))):Yn(l,r,n)}return s.length?Promise.all(s).then(()=>r):r}});function Gt(e,t,r,o,i,s){let n=r in o;if(e.issues.length){if(i&&s&&!n)return;t.issues.push(...Ht(r,e.issues))}if(!n&&!i){e.issues.length||t.issues.push({code:"invalid_type",expected:"nonoptional",input:void 0,path:[r]});return}e.value===void 0?n&&(t.value[r]=void 0):t.value[r]=e.value}function ts(e){let t=Object.keys(e.shape);for(let o of t)if(!e.shape?.[o]?._zod?.traits?.has("$ZodType"))throw new Error(`Invalid element at key "${o}": expected a Zod schema`);let r=Rn(e.shape);return{...e,keys:t,keySet:new Set(t),numKeys:t.length,optionalKeys:new Set(r)}}function rs(e,t,r,o,i,s){let n=[],a=i.keySet,l=i.catchall._zod,f=l.def.type,c=l.optin==="optional",p=l.optout==="optional";for(let g in t){if(g==="__proto__"||a.has(g))continue;if(f==="never"){n.push(g);continue}let h=l.run({value:t[g],issues:[]},o);h instanceof Promise?e.push(h.then(w=>Gt(w,r,g,t,c,p))):Gt(h,r,g,t,c,p)}return n.length&&r.issues.push({code:"unrecognized_keys",keys:n,input:t,inst:s}),e.length?Promise.all(e).then(()=>r):r}var no=K("$ZodObject",(e,t)=>{if(ke.init(e,t),!Object.getOwnPropertyDescriptor(t,"shape")?.get){let a=t.shape;Object.defineProperty(t,"shape",{get:()=>{let l={...a};return Object.defineProperty(t,"shape",{value:l}),l}})}let o=An(()=>ts(t));Ee(e._zod,"propValues",()=>{let a=t.shape,l={};for(let f in a){let c=a[f]._zod;if(c.values){l[f]??(l[f]=new Set);for(let p of c.values)l[f].add(p)}}return l});let i=Ln,s=t.catchall,n;e._zod.parse=(a,l)=>{n??(n=o.value);let f=a.value;if(!i(f))return a.issues.push({expected:"object",code:"invalid_type",input:f,inst:e}),a;a.value={};let c=[],p=n.shape;for(let g of n.keys){let h=p[g],w=h._zod.optin==="optional",u=h._zod.optout==="optional",d=h._zod.run({value:f[g],issues:[]},l);d instanceof Promise?c.push(d.then(b=>Gt(b,a,g,f,w,u))):Gt(d,a,g,f,w,u)}return s?rs(c,f,a,l,o.value,e):c.length?Promise.all(c).then(()=>a):a}});var oo=K("$ZodEnum",(e,t)=>{ke.init(e,t);let r=zn(t.entries),o=new Set(r);e._zod.values=o,e._zod.pattern=new RegExp(`^(${r.filter(i=>Un.has(typeof i)).map(i=>typeof i=="string"?Cr(i):i.toString()).join("|")})$`),e._zod.parse=(i,s)=>{let n=i.value;return o.has(n)||i.issues.push({code:"invalid_value",values:r,input:n,inst:e}),i}});var so=K("$ZodNullable",(e,t)=>{ke.init(e,t),Ee(e._zod,"optin",()=>t.innerType._zod.optin),Ee(e._zod,"optout",()=>t.innerType._zod.optout),Ee(e._zod,"pattern",()=>{let r=t.innerType._zod.pattern;return r?new RegExp(`^(${In(r.source)}|null)$`):void 0}),Ee(e._zod,"values",()=>t.innerType._zod.values?new Set([...t.innerType._zod.values,null]):void 0),e._zod.parse=(r,o)=>r.value===null?r:t.innerType._zod.run(r,o)});function io(e,t){return new e({type:"string",...me(t)})}function ao(e,t){return new e({type:"number",checks:[],...me(t)})}function co(e,t){return new e({type:"number",check:"number_format",abort:!1,format:"safeint",...me(t)})}function uo(e,t){return new e({type:"boolean",...me(t)})}function lo(e){return new e({type:"unknown"})}function Vt(e,t){return new Wn({check:"less_than",...me(t),value:e,inclusive:!0})}function po(e,t){return new Tr({check:"greater_than",...me(t),value:e,inclusive:!1})}function _t(e,t){return new Tr({check:"greater_than",...me(t),value:e,inclusive:!0})}function Er(e){return po(0,e)}function Ir(e,t){return new Vn({check:"min_length",...me(t),minimum:e})}var De=K("ZodMiniType",(e,t)=>{if(!e._zod)throw new Error("Uninitialized schema in ZodMiniType.");ke.init(e,t),e.def=t,e.type=t.type,e.parse=(r,o)=>Kt(e,r,o,{callee:e.parse}),e.safeParse=(r,o)=>xt(e,r,o),e.parseAsync=async(r,o)=>Wt(e,r,o,{callee:e.parseAsync}),e.safeParseAsync=async(r,o)=>bt(e,r,o),e.check=(...r)=>e.clone({...t,checks:[...t.checks??[],...r.map(o=>typeof o=="function"?{_zod:{check:o,def:{check:"custom"},onattach:[]}}:o)]},{parent:!0}),e.with=e.check,e.clone=(r,o)=>Pr(e,r,o),e.brand=()=>e,e.register=((r,o)=>(r.add(e,o),e)),e.apply=r=>r(e)}),ss=K("ZodMiniString",(e,t)=>{Qn.init(e,t),De.init(e,t)});function Ie(e){return io(ss,e)}var fo=K("ZodMiniNumber",(e,t)=>{Ar.init(e,t),De.init(e,t)});function Me(e){return ao(fo,e)}var is=K("ZodMiniNumberFormat",(e,t)=>{qn.init(e,t),fo.init(e,t)});function Lr(e){return co(is,e)}var as=K("ZodMiniBoolean",(e,t)=>{eo.init(e,t),De.init(e,t)});function ae(e){return uo(as,e)}var cs=K("ZodMiniUnknown",(e,t)=>{to.init(e,t),De.init(e,t)});function us(){return lo(cs)}var ls=K("ZodMiniArray",(e,t)=>{ro.init(e,t),De.init(e,t)});function Ur(e,t){return new ls({type:"array",element:e,...me(t)})}var ho=K("ZodMiniObject",(e,t)=>{no.init(e,t),De.init(e,t),Ee(e,"shape",()=>t.shape)});function Rr(e,t){let r={type:"object",shape:e??{},...me(t)};return new ho(r)}function mo(e,t){return new ho({type:"object",shape:e,catchall:us(),...me(t)})}var ps=K("ZodMiniEnum",(e,t)=>{oo.init(e,t),De.init(e,t),e.options=Object.values(t.entries)});function go(e,t){let r=Array.isArray(e)?Object.fromEntries(e.map(o=>[o,o])):e;return new ps({type:"enum",entries:r,...me(t)})}var fs=K("ZodMiniNullable",(e,t)=>{so.init(e,t),De.init(e,t)});function Jt(e){return new fs({type:"nullable",innerType:e})}var ds=new Set(["__proto__","prototype","constructor"]);function st(e){return e!==null&&typeof e=="object"&&!Array.isArray(e)}function Zr(e,t){if(!st(e)||!st(t))return t===void 0?e:t;let r={...e};for(let o of Object.keys(t)){if(ds.has(o))continue;let i=t[o];r[o]=st(e[o])&&st(i)?Zr(e[o],i):i}return r}var le=Ie().check(Ir(1)),Ce=Jt(Ie()),yo=["socks5","http","https","turn"],hs=Jt(go(yo)),ms={全局:Ie(),标准:Ie()};function gs(e,t){return t?mo(e):Rr(e)}function wo(e,{保留未知字段:t=!0,持久化:r=!1}={}){let o=a=>gs(a,t),i=o(ms),s={local:ae(),本地IP库:o({随机IP:ae(),随机数量:Me().check(Lr(),_t(0)),指定端口:Me().check(Lr(),_t(-1),Vt(65535))}),SUB:Ce,SUBNAME:le,SUBUpdateTime:Me().check(Er())};r||(s.TOKEN=le);let n={TIME:Ie(),HOST:le,HOSTS:Ur(le),UUID:le,PATH:le,协议类型:le,传输协议:le,gRPC模式:le,gRPCUserAgent:Ie(),跳过证书验证:ae(),启用0RTT:ae(),TLS分片:Ce,随机路径:ae(),ECH:ae(),ECHConfig:o({DNS:le,SNI:Ce}),SS:o({加密方式:le,TLS:ae()}),Fingerprint:le,优选订阅生成:o(s),订阅转换配置:o({SUBAPI:le,SUBCONFIG:le,SUBEMOJI:ae(),SUBLIST:ae(),UDP:ae(),XUDP:ae(),TLS13:ae(),APPEND_TYPE:ae(),SORT:ae()}),反代:o({[e]:le,SOCKS5:o({启用:hs,全局:ae(),账号:Ie(),白名单:Ur(Ie())}),路径模板:Rr({[e]:le,SOCKS5:i,HTTP:i,HTTPS:i,TURN:i})}),TG:o(r?{启用:ae()}:{启用:ae(),BotToken:Ce,ChatID:Ce})};return r||(n.CF=o({Email:Ce,GlobalAPIKey:Ce,AccountID:Ce,APIToken:Ce,UsageAPI:Ce,Usage:Jt(o({success:ae(),pages:Me(),workers:Me(),total:Me(),max:Me()}))})),o(n)}function xo(e,t,r){if(!st(e))return{success:!1,error:{issues:[{code:"custom",path:[],message:"配置必须是 JSON 对象"}]}};let o=Zr(t,e),i=o?.反代?.SOCKS5?.启用;return i!==null&&!yo.includes(i)&&(o.反代.SOCKS5.启用=null),wo(r).safeParse(o)}function bo(e,t,r){return st(e)?wo(r,{保留未知字段:!1,持久化:!0}).safeParse(Zr(t,e)):{success:!1,error:{issues:[{code:"custom",path:[],message:"配置必须是 JSON 对象"}]}}}function Xt(e){return e.issues.map(t=>({path:t.path.length?t.path.join("."):"$",message:t.message}))}async function Yt(e,t,r,o="Mozilla/5.0",i=!1){let s,n=se[0],a=t,l="https://dns.alidns.com/dns-query",f="cloudflare-ech.com",c="{{IP:PORT}}",p=performance.now(),g={TIME:new Date().toISOString(),HOST:a,HOSTS:[t],UUID:r,PATH:"/",协议类型:"vless",传输协议:"ws",gRPC模式:"gun",gRPCUserAgent:o,跳过证书验证:!1,启用0RTT:!1,TLS分片:null,随机路径:!1,ECH:!1,ECHConfig:{DNS:l,SNI:f},SS:{加密方式:"aes-128-gcm",TLS:!0},Fingerprint:"chrome",优选订阅生成:{local:!0,本地IP库:{随机IP:!0,随机数量:16,指定端口:-1},SUB:null,SUBNAME:"edgetunnel",SUBUpdateTime:3,TOKEN:await ye(t+r)},订阅转换配置:{SUBAPI:`https://SUBAPI.${se[1]}ssss.net`,SUBCONFIG:`https://raw.githubusercontent.com/${se[1]}/ACL4SSR/refs/heads/main/Clash/config/ACL4SSR_Online_Mini_MultiMode_CF.ini`,SUBEMOJI:!1,SUBLIST:!1,UDP:!1,XUDP:!1,TLS13:!1,APPEND_TYPE:!1,SORT:!1},反代:{[n]:"auto",SOCKS5:{启用:null,全局:!1,账号:"",白名单:je()},路径模板:{[n]:"proxyip="+c,SOCKS5:{全局:"socks5://"+c,标准:"socks5="+c},HTTP:{全局:"http://"+c,标准:"http="+c},HTTPS:{全局:"https://"+c,标准:"https="+c},TURN:{全局:"turn://"+c,标准:"turn="+c}}},TG:{启用:!1,BotToken:null,ChatID:null},CF:{Email:null,GlobalAPIKey:null,AccountID:null,APIToken:null,UsageAPI:null,Usage:{success:!1,pages:0,workers:0,total:0,max:1e5}}};try{let z=await e.KV.get("config.json");!z||i==!0?(await e.KV.put("config.json",JSON.stringify(g,null,2)),s=g):s=JSON.parse(z)}catch(z){console.error(`读取config_JSON出错: ${z.message}`),s=g}let h=xo(s,g,n);h.success?s=h.data:(console.error("config.json 配置校验失败，已回退到默认配置:",JSON.stringify(Xt(h.error))),s=g),s.订阅转换配置.SUBLIST||(s.订阅转换配置.SUBLIST=!1),s.订阅转换配置.UDP||(s.订阅转换配置.UDP=!1),s.订阅转换配置.XUDP||(s.订阅转换配置.XUDP=!1),s.订阅转换配置.TLS13||(s.订阅转换配置.TLS13=!1),s.订阅转换配置.APPEND_TYPE||(s.订阅转换配置.APPEND_TYPE=!1),s.订阅转换配置.SORT||(s.订阅转换配置.SORT=!1),s.gRPCUserAgent||(s.gRPCUserAgent=o),s.HOST=a,s.HOSTS||(s.HOSTS=[t]),e.HOST&&(s.HOSTS=(await _e(e.HOST)).map(z=>z.toLowerCase().replace(/^https?:\/\//,"").split("/")[0].split(":")[0])),s.UUID=r,s.随机路径||(s.随机路径=!1),s.启用0RTT||(s.启用0RTT=!1),e.PATH?s.PATH=e.PATH.startsWith("/")?e.PATH:"/"+e.PATH:s.PATH||(s.PATH="/"),s.gRPC模式||(s.gRPC模式="gun"),s.SS||(s.SS={加密方式:"aes-128-gcm",TLS:!1}),s.反代.路径模板?.[n]||(s.反代.路径模板={[n]:"proxyip="+c,SOCKS5:{全局:"socks5://"+c,标准:"socks5="+c},HTTP:{全局:"http://"+c,标准:"http="+c},HTTPS:{全局:"https://"+c,标准:"https="+c},TURN:{全局:"turn://"+c,标准:"turn="+c}}),s.反代.路径模板.HTTPS||(s.反代.路径模板.HTTPS={全局:"https://"+c,标准:"https="+c}),s.反代.路径模板.TURN||(s.反代.路径模板.TURN={全局:"turn://"+c,标准:"turn="+c});let w=s.反代.路径模板[s.反代.SOCKS5.启用?.toUpperCase()],u="";w&&s.反代.SOCKS5.账号?u=(s.反代.SOCKS5.全局?w.全局:w.标准).replace(c,s.反代.SOCKS5.账号):s.反代[n]!=="auto"&&(u=s.反代.路径模板[n].replace(c,s.反代[n]));let d="";if(u.includes("?")){let[z,A]=u.split("?");u=z,d=A}s.PATH=s.PATH.replace(u,"").replace("//","/");let b=s.PATH==="/"?"":s.PATH.replace(/\/+(?=\?|$)/,"").replace(/\/+$/,""),[$,...S]=b.split("?"),m=S.length?"?"+S.join("?"):"",v=d?m?m+"&"+d:"?"+d:m;s.完整节点路径=($||"/")+($&&u?"/":"")+u+v+(s.启用0RTT?(v?"&":"?")+"ed=2560":""),!s.TLS分片&&s.TLS分片!==null&&(s.TLS分片=null);let x=s.TLS分片=="Shadowrocket"?`&fragment=${encodeURIComponent("1,40-60,30-50,tlshello")}`:s.TLS分片=="Happ"?`&fragment=${encodeURIComponent("3,1,tlshello")}`:"";s.Fingerprint||(s.Fingerprint="chrome"),s.ECH||(s.ECH=!1),s.ECHConfig||(s.ECHConfig={DNS:l,SNI:f});let y=s.ECH?`&ech=${encodeURIComponent((s.ECHConfig.SNI?s.ECHConfig.SNI+"+":"")+s.ECHConfig.DNS)}`:"",{type:C,路径字段名:k,域名字段名:E}=Mt(s),T=jt(s,s.完整节点路径);s.LINK=s.协议类型==="ss"?`${s.协议类型}://${btoa(s.SS.加密方式+":"+r)}@${a}:${s.SS.TLS?"443":"80"}?plugin=v2${encodeURIComponent(`ray-plugin;mode=websocket;host=${a};path=${(s.完整节点路径.includes("?")?s.完整节点路径.replace("?","?enc="+s.SS.加密方式+"&"):s.完整节点路径+"?enc="+s.SS.加密方式)+(s.SS.TLS?";tls":"")};mux=0`)+y}#${encodeURIComponent(s.优选订阅生成.SUBNAME)}`:`${s.协议类型}://${r}@${a}:443?security=tls&type=${C+y}&${E}=${a}&fp=${s.Fingerprint}&sni=${a}&${k}=${encodeURIComponent(T)+x}&encryption=none#${encodeURIComponent(s.优选订阅生成.SUBNAME)}`,s.优选订阅生成.TOKEN=await ye(t+r);let P={BotToken:null,ChatID:null};s.TG={启用:s.TG.启用?s.TG.启用:!1,...P};try{let z=await e.KV.get("tg.json");if(!z)await e.KV.put("tg.json",JSON.stringify(P,null,2));else{let A=JSON.parse(z);s.TG.ChatID=A.ChatID?A.ChatID:null,s.TG.BotToken=A.BotToken?ct(A.BotToken):null}}catch(z){console.error(`读取tg.json出错: ${z.message}`)}let _={Email:null,GlobalAPIKey:null,AccountID:null,APIToken:null,UsageAPI:null};s.CF={..._,Usage:{success:!1,pages:0,workers:0,total:0,max:1e5}};try{let z=await e.KV.get("cf.json");if(!z)await e.KV.put("cf.json",JSON.stringify(_,null,2));else{let A=JSON.parse(z);if(A.UsageAPI)try{let U=await(await fetch(A.UsageAPI)).json();s.CF.Usage=U}catch(L){console.error(`请求 CF_JSON.UsageAPI 失败: ${L.message}`)}else{s.CF.Email=A.Email?A.Email:null,s.CF.GlobalAPIKey=A.GlobalAPIKey?ct(A.GlobalAPIKey):null,s.CF.AccountID=A.AccountID?ct(A.AccountID):null,s.CF.APIToken=A.APIToken?ct(A.APIToken):null,s.CF.UsageAPI=null;let L=await Ft(A.Email,A.GlobalAPIKey,A.AccountID,A.APIToken);s.CF.Usage=L}}}catch(z){console.error(`读取cf.json出错: ${z.message}`)}return s.加载时间=(performance.now()-p).toFixed(2)+"ms",s}async function Le(e,t,r,o="Get_SUB",i,s=!0){try{let n=new Date,a={TYPE:o,IP:r,ASN:`AS${t.cf.asn||"0"} ${t.cf.asOrganization||"Unknown"}`,CC:`${t.cf.country||"N/A"} ${t.cf.city||"N/A"}`,URL:t.url,UA:t.headers.get("User-Agent")||"Unknown",TIME:n.getTime()};if(i.TG.启用)try{let p=await e.KV.get("tg.json"),g=JSON.parse(p);if(g?.BotToken&&g?.ChatID){let h=new Date(a.TIME).toLocaleString("zh-CN",{timeZone:"Asia/Shanghai"}),w=new URL(a.URL),u=`<b>#${i.优选订阅生成.SUBNAME} 日志通知</b>
...
`)),s}async function So(){return`<!DOCTYPE html>
<html lang="zh-CN">
<head>
	<meta charset="UTF-8">
